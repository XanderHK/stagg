    ...
    return (
        <Template title="Call of Duty Warzone Barracks" renderReport={renderReport}>
            <div className="container text-center" style={{paddingTop: 64}}>
                <Reports.Barracks.Component { ...reportProps } />
            </div>
        </Template>
    )
}
...
Page.getInitialProps = async ({ store, res, req, query }:NextPageContext) => {
    ...
    const reportProps = await Reports.Barracks.PropsLoader({ accountIdentifier: { uno: Model.CallOfDuty.format.username.raw(unoUsername) } }, limit, skip)
    return { reportProps }
}
